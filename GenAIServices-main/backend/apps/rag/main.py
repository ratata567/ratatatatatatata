...
def query_doc_handler(
    ...
    try:
        ...
        if collection_name == None:
            temp_collection_name = user.id
        else:
            temp_collection_name = collection_name + user.id
...
def query_collection_handler(
    ...
    try:
        ...
        for collection_name in collection_names:
            if collection_name == None:
                temp_collection_names.append(user.id)
            else:
                temp_collection_names.append(collection_name + user.id)
        print(temp_collection_names)
        if app.state.config.ENABLE_RAG_HYBRID_SEARCH:
            return query_collection_with_hybrid_search(
                collection_names=temp_collection_names,
                query=form_data.query,
                embedding_function=app.state.EMBEDDING_FUNCTION,
                k=form_data.k if form_data.k else app.state.config.TOP_K,
                reranking_function=app.state.sentence_transformer_rf,
...
def store_web(form_data: UrlForm, user=Depends(get_verified_user)):
    ...
    try:
        ...
        store_data_in_vector_db(data, temp_collection_name, overwrite=True)
        return {
            "status": True,
            "collection_name": collection_name,
            "filename": form_data.url,
...
def store_doc(
    ...
    try:
        ...
        try:
            result = store_data_in_vector_db(data, temp_collection_name)
...
def delete_doc(
    ...
    try:
        ...
        try:
            result = delete_docs_from_vector_db(temp_collection_name, vector_ids)
