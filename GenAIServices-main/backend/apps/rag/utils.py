...
def get_rag_context(
    ...
    for file in files:
        ...
        print("Here#")
        print(files)
        def collection_names_transformer(collection_names, user_id):
            return_val = []
            for collection_name in collection_names:
                return_val.append(collection_name+user_id)
            return return_val
        
        collection_names = (
            collection_names_transformer(file['collection_names'], file['user_id'])
            if file["type"] == "collection"
            else [(file["collection_name"]+file["user_id"])]
        )
        print("Here2#")
